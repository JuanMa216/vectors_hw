...
// Función para eliminar duplicados con medición de tiempo
template <typename T>
Vector<T> removeDuplicates(const Vector<T>& vector) {
    ...
    auto end = high_resolution_clock::now(); // Finalizar medición de tiempo
...
int main() {
    ...
    Vector<int> numbers2 = {1,1,1,1,1,1,1};
    Vector<int> uniqueNumbers2 = removeDuplicates(numbers2);
    uniqueNumbers2.print(); // Expected: {1, 7}

    Vector<int> numbers3 = {1,2,3,4,5,6,7};
    Vector<int> uniqueNumbers3 = removeDuplicates(numbers3);
    uniqueNumbers3.print(); // Expected: {1, 2, 3, 4, 5, 6, 7}
